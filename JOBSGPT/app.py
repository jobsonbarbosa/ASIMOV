...
# Função para obter resposta do modelo OpenAI
...
def desconverte_nome_mensagem(nome_arquivo, key='mensagem'):
...
#Lenddo mensagens
def ler_mensagem(mensagens, key='mensagem'):
    if len(mensagens) == 0:
        return []
    nome_mensagem = retorna_nome_da_mensagem(mensagens)
    nome_arquivo = converte_nome_mensagem(nome_mensagem)
...
def pagina_principal():
    ...
    mensagens = ler_mensagem(st.session_state['mensagens'], key='mensagem')

    st.header('🤖 Jobs Chatbot', divider=True)
